...
2. The new behavior "**The first subdirectory of `path` is replaced by everything in `location`**" (see **IMPORTANT** note above) is quite opinionated, though not necessarily a bad thing. Are there better ways to generalize this? Consider the following example scenarios and the results with the current implementation of this feature enhancement. These results may or may not be too intuitive... 
